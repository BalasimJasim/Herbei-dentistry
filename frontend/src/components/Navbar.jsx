...
const Navbar = () => {
  ...
  const isActive = (path) => {
    ...
    return currentPath === path || 
           ...
           currentPath === path.slice(1) // handle paths without leading slash
  }
  ...
  const navLinks = [
    ...
    ...(user?.role === 'admin' ? [{ path: '/admin/dashboard', text: 'Admin Dashboard' }] : [])
  ]
    ...
    <nav className={`navbar ${isScrolled ? 'scrolled' : ''}`}>
      <div className="navbar-container">
        <div className="navbar-brand">
          <Link to="/">
            <img src="/logo.svg" alt="Herbie Dental Clinic" className="navbar-logo" />
            <h1>Herbie Dental</h1>
          </Link>
        </div>
        ...
        <div className={`nav-menu ${isOpen ? 'active' : ''}`}>
          <ul className="nav-links">
            ...
            <li>
              <Link 
                to="/patient-portal" 
                className={`portal-btn ${isActive('/patient-portal') ? 'active' : ''}`}
              >
                {t('nav.patientPortal')}
              </Link>
            </li>
